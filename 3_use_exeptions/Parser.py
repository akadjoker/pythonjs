...
class Parser:
    ...
    def loop_statement(self):
        ...
        condition = Literal(1)  # Condição sempre verdadeira para o loop infinito
        return WhileStmt(condition, BlockStmt(body))  # Encapsule o corpo em um BlockStmt
    ...
    def for_statement(self):# sintatic sugar
        self.consume(TokenType.LPAREN, "Expect '(' after 'for'.")
        initializer = None
        if self.match(TokenType.IDINT, TokenType.IDFLOAT):
            initializer = self.var_declaration()
    ...
    def break_statement(self):
        return BreakStmt(self.previous())

    def continue_statement(self):
        return ContinueStmt(self.previous())
